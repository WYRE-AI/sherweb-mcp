...
* align package name to sherweb-mcp convention ([7efe4c5](https://github.com/WYRE-AI/sherweb-mcp/commit/7efe4c56cbd09c9e0bb4142f5a71a9d5eb4be676))


### Features

* initial scaffold with lazy loading support ([aa375d0](https://github.com/WYRE-AI/sherweb-mcp/commit/aa375d00e2d9d961a601578ee2437a5567e9e755))
...
### Fixed
- **API error messages now name the request that failed.** Investigating a 2026-09-08 report from Epion (`sherweb_customers_list` → `Sherweb API error (500): HTTP 500`, `sherweb_catalog_list_products` → `Not found: HTTP 404`, `sherweb_billing_payable_charges` → `Not found: HTTP 404`) required re-deriving, from git history, that all three symptoms are byte-for-byte what the *pre-#67* code produced: `/customers` with unsupported `page`/`pageSize`/`search` query params Sherweb's `GetCustomers` does not document (500, most likely from those params reaching an endpoint that doesn't expect them), `/catalog/products` (404, endpoint never existed), and `/payable-charges` instead of `/billing/payable-charges` (404). Current `main` already contains the fix (#67, released 2026-08-27 as v2.0.0) — independently re-verified here against `developers.sherweb.com/reference` for `GetCustomers`, `GetCustomerCatalog` and `GetPayableCharges` — so there was no remaining code defect to fix for any of the three tools. What made root-causing this slow was that `handleApiError` (`src/utils/client.ts`) only logged the failing method + URL to stderr; the *thrown* message a caller sees carried just the bare status (`HTTP 404`, `HTTP 500`), indistinguishable from a live upstream issue. Every thrown error now appends the request line, e.g. `Not found: HTTP 404 (GET https://api.sherweb.com/service-provider/v1/customer-catalogs/bad-id)` — so a stale deployment still hitting a dead pre-#67 path is now identifiable from the error text alone, no log access or git archaeology required. Covered by new tests in `src/utils/client.test.ts` pinning the message shape for 401/403/404/429 and the generic status branch, including the pre-#67 unsupported-query-params shape.
  - **Correction: the "deployment lag" hypothesis below was wrong.** A direct query against `conduit-prod`'s live Log Analytics workspace confirmed the deployed Container App (`sherweb-mcp` in `rg-conduit-prod`, revision `sherweb-mcp--0000006`, image digest `sha256:a09b5a3581f6ea37092751fbbc056f585a9034482cbeec303f15aa202d97ab70`, the correct v2.0.0 pin) **is** running the fixed endpoint paths. The 2026-09-08 recurrence was two different, still-live bugs, not stale endpoints — see the two entries below.
  - ~~**This incident's actual cause was deployment lag, not a code bug — flagging for the conduit side.**~~ `sherweb-mcp`'s `main` has had the correct endpoints since 2026-08-27, and `conduit`'s vendor-fleet pin for `sherweb` was bumped to that build's v2.0.0 image on 2026-09-04 (`vendor-fleet.conduit-prod.bicepparam`, PR conduit#1609). Despite both, the 2026-09-08 report reproduced exactly the pre-fix symptoms, which at the time looked like the running Container App revision hadn't actually rolled out that pin — but live evidence (above) ruled that out.
- **`sherweb_catalog_list_products` could send the literal string `"undefined"` as a URL path segment.** `handleCall` destructured `customerId` from `args` with a bare `as` type assertion and no runtime check, so a caller that omitted the (declared-`required`) `customerId` argument got a real JS `undefined`, which `` `/customer-catalogs/${customerId}` `` string-interpolated into the literal request `/customer-catalogs/undefined` — reproduced verbatim in a 2026-09-08 production log from Epion, which got back an opaque `500` instead of a validation error. Per Sherweb's own `GetCustomerCatalog` reference (`developers.sherweb.com/reference/getcustomercatalog`), `customerId` "represents a specific customer under your account" — i.e. one of the caller's own end-customers (plural, discoverable via `sherweb_customers_list`), not a single self-lookup ID the server could resolve on the caller's behalf. So the fix is client-side validation, not auto-resolution: a new `requireString()` helper (`src/utils/types.ts`) now rejects a missing/empty/non-string required argument with a clear error *before* any request is built, e.g. `sherweb_catalog_list_products: missing required argument 'customerId'. Provide a non-empty string.` The same defect class (a required argument interpolated directly into a URL path with no validation) also existed in `sherweb_subscriptions_amendment_status`'s `` `/tracking/${trackingId}` `` and is fixed the same way. New tests in `src/domains/endpoints.test.ts` assert both tools reject a missing/wrong-typed required argument without making any upstream request.
- **`sherweb_customers_list` (and every other Service Provider/Distributor call) requested an OAuth scope Sherweb's own tooling never uses.** `authenticate()` asked for the combined scope `"distributor service-provider"` on every token exchange, regardless of which API the token was actually for. Sherweb's Authorization API OpenAPI spec says plainly: "you need to pass a scope depending of which API you are gonna call afterwards" — and its official sample code plus Postman collection (`github.com/sherweb/Public-Apis`) both request one bare scope value matching the destination API (e.g. `"distributor"` for the Distributor API), never a combined string. `authenticate()` now takes an explicit `scope` argument, and `distributorRequest`/`serviceProviderRequest` each request only their own API's scope (`"distributor"` / `"service-provider"`); the token cache key now includes scope so a tenant calling both APIs gets two independently-cached tokens instead of one shared (and potentially wrongly-scoped) token. New tests in `src/utils/client.test.ts` pin the per-API scope value sent to the token endpoint and confirm calling both APIs for one tenant authenticates twice. **This is the most likely code-side explanation for `sherweb_customers_list`'s 500** — Sherweb's `GetCustomers` reference documents no query parameters, no required scope beyond a valid bearer token, and no 500 conditions at all, and the request itself already matches the documented contract exactly, so a scope value with no precedent in any Sherweb sample or spec is the strongest lead found. **Not smoke-tested against a live 200** — no Sherweb credentials are available in the secret store (`cortex-secret list` confirms) — so if this does not resolve the 500 in production, the next step is confirming directly with Sherweb support whether `"distributor service-provider"` is rejected, silently narrowed, or otherwise mishandled server-side.
